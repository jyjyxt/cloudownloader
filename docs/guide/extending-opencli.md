...
|------|-----|-----------------|-----------------|
| Build a personal website command in your own Git repo | Local plugin | Your project directory, symlinked into `~/.opencli/plugins/` | `ClouDownloader <plugin> <command>` |
| Quickly draft a private adapter on this machine | User adapter | `~/.opencli/clis/<site>/<command>.js` | `ClouDownloader <site> <command>` |
| Edit an official adapter locally | Adapter override | `~/.opencli/clis/<site>/` | `ClouDownloader <site> <command>` |
| Publish or install third-party commands | Plugin | Git repo, installed into `~/.opencli/plugins/` | `ClouDownloader <plugin> <command>` |
| Wrap an existing local binary | External CLI | `~/.opencli/external-clis.yaml` | `ClouDownloader <tool> ...` |
...
```bash
ClouDownloader plugin create my-cnn
cd my-cnn
git init
ClouDownloader plugin install file://$(pwd)
ClouDownloader my-cnn hello
```
...
```bash
ClouDownloader browser init cnn/top
# edit ~/.opencli/clis/cnn/top.js
ClouDownloader browser verify cnn/top
ClouDownloader cnn top
```
...
```bash
ClouDownloader browser verify instagram/collection-create --write-fixture --seed-args opencli-verify
ClouDownloader browser verify example/detail --write-fixture --seed-args '["https://example.com/item/1", "--limit", 3]'
```

`--seed-args` is only used when the fixture has no `args`. Once the fixture is written, `ClouDownloader browser verify` reads args from `~/.opencli/sites/<site>/verify/<command>.json`.
...
```bash
ClouDownloader adapter eject twitter
# edit ~/.opencli/clis/twitter/*.js
ClouDownloader adapter reset twitter
```

Files in `~/.opencli/clis/<site>/<command>.js` override packaged adapters with the same `site/command` on this machine. `ClouDownloader browser verify <site>/<command>` also runs the local override, so a passing local verify does not prove that the packaged adapter was changed.
...
After copying a local fix into the repository for a PR, remove the local copy or run `ClouDownloader adapter reset <site>` after merge. Otherwise the local file keeps shadowing future package updates. `ClouDownloader doctor` warns when it detects this shadowing.
...
ClouDownloader plugin install https://github.com/user/opencli-plugin-my-tool
ClouDownloader plugin install file:///absolute/path/to/plugin
...
ClouDownloader plugin install github:user/opencli-plugins/cnn
```

For local development, install each sub-plugin directory directly:

```bash
ClouDownloader plugin install file:///absolute/path/opencli-plugins/packages/cnn
ClouDownloader plugin install file:///absolute/path/opencli-plugins/packages/reuters
```
...
Use external CLI registration when the command already exists as a binary on your machine and you want it available through `ClouDownloader`.
