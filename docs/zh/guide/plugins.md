...
Cloudl 支持社区贡献的 plugins。你可以从 GitHub 安装第三方 adapters，它们会和内置 commands 一起在启动时自动发现。
...
Plugins 存放在 `~/.cloudl/plugins/<name>/`。每个子目录都会在启动时扫描 `.ts`、`.js` 命令文件，格式与内置 adapters 相同。
...
如果仓库名带 `cloudl-plugin-` 前缀，本地目录会自动去掉这个前缀。例如 `cloudl-plugin-hot-digest` 会变成 `hot-digest`。

## 插件清单 (`cloudl-plugin.json`)

插件可以在仓库根目录放置 `cloudl-plugin.json` 来声明元数据：
...
| `version` | 语义化版本 |
| `cloudl` | 所需的 cloudl 版本范围（如 `>=1.0.0`、`^1.2.0`） |
| `description` | 描述 |
...
一个仓库可以通过在 `cloudl-plugin.json` 中声明 `plugins` 字段来包含多个插件：
...
- Monorepo 只 clone 一次到 `~/.cloudl/monorepos/<repo>/`
- 每个子插件通过 symlink 出现在 `~/.cloudl/plugins/<name>/`
- 更新任何子插件会拉取整个 monorepo 并刷新所有子插件
- 卸载最后一个子插件时，monorepo 目录会被自动清理

## 版本追踪

Cloudl 会把已安装 plugin 的版本记录到 `~/.cloudl/plugins.lock.json`。每条记录会保存 plugin source、当前 git commit hash、安装时间，以及最近一次更新时间。只要有这份元数据，`cloudl plugin list` 就会显示对应的短 commit hash。
...
3. 将宿主 `cloudl` 链接到 plugin 的 `node_modules/`，保证 `@jyjyxt/cloudl/registry` 指向当前宿主版本

## 示例 plugins

使用 `cloudl plugin create my-plugin` 创建插件。第三方插件需先更新为 `@jyjyxt/cloudl` 包导入和 `cloudl-plugin.json` 清单，再安装使用。

## 排查问题

### TS plugin import 报错

如果看到 `Cannot find module '@jyjyxt/cloudl/registry'`，通常是宿主 symlink 失效。重新安装 plugin 即可：
