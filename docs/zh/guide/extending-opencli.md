...
| 目标 | 使用方式 | 源码位置 | 命令入口 |
|------|----------|----------|----------|
| 在自己的 Git repo 里写个人网站命令 | 本地 plugin | 你的项目目录，symlink 到 `~/.opencli/plugins/` | `cloudl <plugin> <command>` |
| 快速写一个只在本机用的 adapter | User adapter | `~/.opencli/clis/<site>/<command>.js` | `cloudl <site> <command>` |
| 本地修改官方 adapter | Adapter override | `~/.opencli/clis/<site>/` | `cloudl <site> <command>` |
| 发布或安装第三方命令 | Plugin | Git repo，安装到 `~/.opencli/plugins/` | `cloudl <plugin> <command>` |
| 包装已有本机 binary | External CLI | `~/.opencli/external-clis.yaml` | `cloudl <tool> ...` |
...
`--seed-args` 只在 fixture 没有 `args` 时生效。fixture 写出后，`cloudl browser verify` 会从 `~/.opencli/sites/<site>/verify/<command>.json` 读取 args。
...
`~/.opencli/clis/<site>/<command>.js` 会在本机覆盖同名 package adapter。`cloudl browser verify <site>/<command>` 也会跑本地覆盖版本，所以本地 verify 通过不代表 package 里的 adapter 已经改好。
