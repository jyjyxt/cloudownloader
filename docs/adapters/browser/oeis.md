...
| Command | Description |
|---------|-------------|
| `cloudl oeis search <query>` | Search OEIS sequences by keyword or numeric pattern |
| `cloudl oeis sequence <id>` | Full OEIS sequence detail by A-number |

## Usage Examples

```bash
# Keyword search
cloudl oeis search fibonacci
cloudl oeis search "prime gaps" --limit 5

# Numeric pattern search (find sequences matching a prefix)
cloudl oeis search "1,1,2,3,5,8"            # Fibonacci
cloudl oeis search "2,3,5,7,11,13,17"       # primes

# Sequence detail (A-number round-trips from search)
cloudl oeis sequence A000045                  # Fibonacci
cloudl oeis sequence A000040                  # Primes
cloudl oeis sequence A000041                  # Partitions
```
