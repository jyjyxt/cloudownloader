...
the visible Mercury reimbursements page through Cloudl Browser Bridge. It
...
- `status: "needs_login"` means Mercury redirected to login; sign into Mercury
  in that Chrome/Cloudl profile and rerun.
...
npm run dev -- mercury reimbursement-plan \
  ...
  --currency USD \
  --date 2026-06-30 \
  --merchant "Cloudl Test Merchant" \
  --category "Office Supplies & Equipment" \
  --notes "Cloudl adapter smoke test; do not submit." \
  -f json
```
...
- **Login is required.** This adapter uses the selected Cloudl browser profile;
...
- `needs_login`: open Mercury in the same Chrome/Cloudl profile, finish login,
