...
| Command | Description |
|---------|-------------|
| `cloudl nuget search <query>` | Search NuGet packages by keyword |
| `cloudl nuget package <id>` | Full NuGet package version history (catalog entries) |

## Usage Examples

```bash
# Keyword search
cloudl nuget search newtonsoft
cloudl nuget search "asp.net core" --limit 10
cloudl nuget search serilog --prerelease=true

# Full version history (id round-trips from search)
cloudl nuget package Newtonsoft.Json
cloudl nuget package Serilog
cloudl nuget package Microsoft.Extensions.Logging
```
