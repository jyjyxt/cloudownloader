...
|---------|------|-------------|
| `cloudl 12306 stations` | Public | Search the station bundle by Chinese name, telecode, full pinyin, or short alias |
| `cloudl 12306 trains` | Public | Trains between two stations on a given date with per-seat-class availability |
| `cloudl 12306 train` | Public | Stop list (arrive / depart / stopover time) for one train segment |
| `cloudl 12306 price` | Public | Ticket prices by seat class for one train segment + date |
| `cloudl 12306 me` | Browser (cookie) | Logged-in account summary (sensitive fields masked by default) |
| `cloudl 12306 passengers` | Browser (cookie) | Saved passenger list (sensitive fields masked by default) |
| `cloudl 12306 orders` | Browser (cookie) | In-progress orders (not yet ridden / refunded / completed) |

## Usage Examples

```bash
# Search stations
cloudl 12306 stations 上海 --limit 5
cloudl 12306 stations beijing
cloudl 12306 stations AOH        # by telecode

# List trains between two stations
cloudl 12306 trains 北京 上海 --date 2026-05-22 --limit 20
cloudl 12306 trains BJP AOH --date 2026-05-22

# Show stops for one train
cloudl 12306 train 24000000G10L --from 北京南 --to 上海虹桥 --date 2026-05-22

# Ticket prices by seat class
cloudl 12306 price 24000000G10L --from 北京南 --to 上海虹桥 --date 2026-05-22

# Authenticated commands (require login on kyfw.12306.cn first)
cloudl 12306 me
cloudl 12306 me --include-sensitive
cloudl 12306 passengers --limit 10
cloudl 12306 orders
cloudl 12306 orders --include-sensitive

# JSON output
cloudl 12306 trains 北京 上海 --date 2026-05-22 -f json
```
...
logged-in 12306 session in the same Chrome instance that Cloudl's bridge
