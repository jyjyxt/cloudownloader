...
| Command | Description |
|---------|-------------|
| `cloudl powerchina search "<query>" --limit <n>` | Search PowerChina procurement notices with API-first extraction and browser fallback |

## Usage Examples

```bash
# Search by keyword
cloudl powerchina search "procurement" --limit 20 -f json

# Search with another keyword
cloudl powerchina search "substation" --limit 10 -f json
```
