...
Built on [OpenCLI](https://github.com/jackwener/OpenCLI), with the `ClouDownloader` command. Compatible with OpenCLI extensions, configuration directories, and plugin APIs.
...
- **Let AI Agents operate any website** — install the `opencli-browser` skill in your AI agent (Claude Code, Cursor, etc.), and it can navigate, click, type/fill, extract, and inspect any page through your logged-in browser via `ClouDownloader browser` primitives.
- **Write new adapters** end-to-end with `ClouDownloader browser` + the `opencli-adapter-author` skill, which guides from first recon through field decoding, code, and `ClouDownloader browser verify`.
...
```bash
ClouDownloader profile list
ClouDownloader profile rename <contextId> work
ClouDownloader profile use work
ClouDownloader --profile work browser main state
```
...
```bash
ClouDownloader list
ClouDownloader hackernews top --limit 5
ClouDownloader bilibili hot --limit 5
```
...
- `ClouDownloader list` shows every registered command.
- `ClouDownloader <site> <command>` runs a built-in or generated adapter.
- `ClouDownloader external register mycli` exposes a local CLI through the same discovery surface.
- `ClouDownloader doctor` helps diagnose browser connectivity.
...
|------|------------------|
| Keep personal website commands in your own Git repo | `ClouDownloader plugin create` + `ClouDownloader plugin install file://...` |
| Quickly draft a private local adapter | `ClouDownloader browser init <site>/<command>` in `~/.opencli/clis/` |
| Modify an official adapter locally | `ClouDownloader adapter eject <site>` + `ClouDownloader adapter reset <site>` |
| Publish or install third-party commands | `ClouDownloader plugin install github:user/repo` |
| Wrap an existing local binary | `ClouDownloader external register <name>` |
...
| **opencli-autofix** | Repair a broken adapter when a built-in command fails | "`ClouDownloader zhihu hot` is returning empty — fix it" |
...
The agent handles all the `ClouDownloader browser` commands internally — you just describe what you want done in natural language.
...
`ClouDownloader browser` commands require a `<session>` positional immediately after `browser`. `ClouDownloader browser work open <url>` and `ClouDownloader browser work tab new [url]` both return a target ID. Use `ClouDownloader browser work tab list` to inspect target IDs, then pass `--tab <targetId>` to route a command to a specific tab. `tab new` creates a new tab without changing the default browser target; only `tab select <targetId>` promotes that tab to the default target for later untargeted commands in the same session.
...
5. `ClouDownloader browser recon analyze <url>` → `ClouDownloader browser recon init <site>/<name>` → write adapter → `ClouDownloader browser recon verify <site>/<name>`.
...
`ClouDownloader browser *` requires an explicit `<session>` positional, uses a foreground browser window by default, and keeps that session's tab lease until `ClouDownloader browser <session> close` or idle cleanup. Browser-backed adapters use a background adapter window and release one-shot tab leases by default. Interactive adapters can declare `siteSession: 'persistent'` to keep a stable site tab for continuity; pass `--site-session ephemeral` for a one-shot tab.
...
Unified passthrough for your existing command-line tools. Run `ClouDownloader <tool> ...` for any of:
...
Register your own with `ClouDownloader external register <name>`; list everything with `ClouDownloader external list`.
...
```bash
ClouDownloader xiaohongshu download "https://www.xiaohongshu.com/search_result/<id>?xsec_token=..." --output ./xhs
ClouDownloader xiaohongshu download "https://xhslink.com/..." --output ./xhs
ClouDownloader rednote download "https://www.rednote.com/search_result/<id>?xsec_token=..." --output ./rednote
ClouDownloader bilibili download BV1xxx --output ./bilibili
ClouDownloader twitter download elonmusk --limit 20 --output ./twitter
ClouDownloader 1688 download 841141931191 --output ./1688-downloads
ClouDownloader xiaoyuzhou download 69b3b675772ac2295bfc01d0 --output ./xiaoyuzhou
ClouDownloader xiaoyuzhou transcript 69dd0c98e2c8be31551f6a33 --output ./xiaoyuzhou-transcripts
```

`ClouDownloader xiaoyuzhou download` and `transcript` require local Xiaoyuzhou credentials in `~/.opencli/xiaoyuzhou.json`.
...
```bash
ClouDownloader bilibili hot -f json    # Pipe to jq or LLMs
ClouDownloader bilibili hot -f csv     # Spreadsheet-friendly
ClouDownloader bilibili hot -v         # Verbose: show pipeline debug steps
```

## Exit Codes

ClouDownloader follows Unix `sysexits.h` so CI / scripts can branch on failure mode: `0` success, `66` empty result, `69` Browser Bridge down, `75` timeout, `77` auth required, `78` config error, `130` Ctrl-C. Full reference: [docs/guide/exit-codes.md](./docs/guide/exit-codes.md).
...
```bash
ClouDownloader plugin install github:user/opencli-plugin-my-tool
ClouDownloader plugin list
ClouDownloader plugin update --all
ClouDownloader plugin uninstall my-tool
```
