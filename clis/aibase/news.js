...
function normalizeLimit(value) {
    ...
    if (!Number.isInteger(limit) || limit <= 0) {
        throw new ArgumentError('limit must be a positive integer', `Example: cloudl aibase news --limit ${DEFAULT_LIMIT}`);
    }
    if (limit > MAX_LIMIT) {
        throw new ArgumentError(`limit must be <= ${MAX_LIMIT}`, `Example: cloudl aibase news --limit ${MAX_LIMIT}`);
    }
    return limit;
}
