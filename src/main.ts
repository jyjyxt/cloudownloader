#!/usr/bin/env node
/**
 * cloudl — Make any website your CLI. AI-powered.
 */
...
if (typeof (globalThis as { Bun?: unknown }).Bun === 'undefined' && !isSupportedNodeVersion(process.version)) {
  process.stderr.write(
    [
      `cloudl requires Node.js >= ${MIN_SUPPORTED_NODE_MAJOR}.0.0.`,
...
// e.g. `cloudl --version` or `cloudl -V`, but NOT `cloudl gh --version`
...
// Rewrite `cloudl browser <session> <subcommand> ...` so commander (which
// can't combine a parent positional with subcommand dispatch) sees the internal
// `--session <name>` flag form. Also refuses the retired `cloudl browser
