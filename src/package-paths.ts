...
export function getBuiltEntryCandidates(
  ...
  try {
    ...
    } else if (pkg.bin && typeof pkg.bin === 'object' && typeof (pkg.bin.ClouDownloader ?? pkg.bin.opencli) === 'string') {
      candidates.push(path.join(packageRoot, pkg.bin.ClouDownloader ?? pkg.bin.opencli));
    }
