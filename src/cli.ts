...
async function resolveBrowserTargetInSession(
  ...
  try {
    ...
    throw new Error(
      ...
      'The Browser Bridge session may have restarted; re-run "cloudl browser tab list" and choose a current target.',
      { cause: err },
    );
  }
...
function getBrowserSession(command?: Command): string {
  // The CLI surface is `cloudl browser <session> <subcommand>`. main.ts rewrites
  ...
  throw new Error('<session> is a required positional argument: cloudl browser <session> <command>');
}
...
export function createProgram(BUILTIN_CLIS: string, USER_CLIS: string): Command {
  ...
  program
    ...
    .action((opts) => {
      ...
      console.log();
      console.log('  cloudl' + ' — available commands');
      console.log();
...
Examples:
  $ cloudl browser work open https://x.com
  $ cloudl browser work open https://x.com --window background
  $ cloudl browser work click 12
  $ cloudl browser work state
  $ cloudl browser work bind
  $ cloudl browser work unbind
`);
  ...
  function emitJavaScriptDialogError(message: string): void {
    console.log(JSON.stringify({
      error: {
        ...
        message,
        hint: 'Handle the modal first: cloudl browser dialog accept (or dismiss). Use --text for prompt dialogs.',
      },
    }, null, 2));
  }
      ...
      if ((!opts.css || typeof opts.css !== 'string') && !locator) {
        console.log(JSON.stringify({
          error: {
            ...
            hint: 'Examples: cloudl browser find --css ".btn.primary"; cloudl browser find --role button --name Save',
          },
        }, null, 2));
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
    ...
    if (inputs.length === 0) {
      return {
        error: {
          ...
          hint: 'Example: cloudl browser upload "input[type=file]" ./receipt.pdf',
        },
      };
    }
    const files: string[] = [];
...
cli({
  ...
  example: 'cloudl ${site} ${command} -f yaml',
  domain: '${domain}',
...
`;
        ...
        console.log('First time on this site? Run: cloudl browser analyze <url>');
        console.log(`Edit the file to implement your adapter, then run: cloudl browser verify ${name}`);
      } catch (err) {
    ...
    .action(async (name: string, opts: { fixture?: boolean; writeFixture?: boolean; updateFixture?: boolean; strictMemory?: boolean; seedArgs?: string; trace?: string } = {}) => {
      try {
        ...
        try {
          ...
          console.log(`  Executing: cloudl ${site} ${command} ${argDisplay}\n`);
  ...
  program
    .command('doctor')
    .description('Diagnose cloudl browser bridge connectivity')
  ...
  const pluginCmd = program.command('plugin').description('Manage cloudl plugins');
  ...
  pluginCmd
    ...
    .action(async (opts) => {
      ...
      if (plugins.length === 0) {
        console.log('  No plugins installed.');
        console.log('  Install one with: cloudl plugin install github:user/repo');
        return;
      }
      if (opts.format === 'json') {
  ...
  pluginCmd
    ...
    .action(async (name: string, opts: { dir?: string; description?: string }) => {
      ...
      try {
        ...
        console.log(`    cloudl plugin install file://${result.dir}`);
        console.log(`    cloudl ${name} hello`);
      } catch (err) {
        console.error(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  profileCmd
    ...
    .action(async () => {
      ...
      if (!status) {
        console.log('Daemon is not running. Run cloudl doctor after opening Chrome.');
        return;
      }
      if (isDaemonStale(status, PKG_VERSION) || !Array.isArray(status.profiles)) {
        ...
        console.log('Run: cloudl daemon restart');
        return;
      }
  ...
  profileCmd
    ...
    .argument('<contextId>', 'Profile contextId from cloudl profile list')
  ...
  // ── Built-in: daemon ──────────────────────────────────────────────────────
  const daemonCmd = program.command('daemon').description('Manage the cloudl daemon');
  // Snapshot before applyRootSubcommandSummaries() rewrites .description() to a child-name listing.
  ...
  // line is accurate: `Usage: cloudl browser <session> click [target] [options]`
  // instead of `cloudl browser click [target] [options]`. Commander does NOT
  ...
  program.on('command:*', (operands: string[]) => {
    ...
    if (isBinaryInstalled(binary)) {
      console.error(`  Tip: '${binary}' exists on your PATH. Use 'cloudl external register ${binary}' to add it as an external CLI.`);
    }
    program.outputHelp();
    process.exitCode = EXIT_CODES.USAGE_ERROR;
  });
...
export function resolveBrowserVerifyInvocation(opts: {
  ...
  if (!fileExists(sourceEntry)) {
    throw new Error(`Could not find cloudl entrypoint under ${projectRoot}. Expected built entry from package.json or src/main.ts.`);
  }
