 ...
 * 2. Ref path: cascading match levels (see below), using data-cloudl-ref
...
export function resolveTargetJs(ref: string, opts: ResolveOptions = {}): string {
  ...
  return `
    (() => {
      ...
      const identity = window.__cloudl_ref_identity || {};
      ...
      if (isNumeric) {
        ...
        // snapshot + find populate into \`__cloudl_ref_identity\`. Kept inline
        ...
        let el = document.querySelector('[data-cloudl-ref="' + ref + '"]');
        ...
        if (!el) {
          ...
          if (recovered) {
            try {
              recovered.setAttribute('data-cloudl-ref', ref);
          ...
          return {
            ...
            hint: 'The element may have been removed. Re-run \`cloudl browser state\` to get a fresh snapshot.',
          };
        }
        ...
        // data-cloudl-ref attribute on the surviving node.
        const recovered = reidentify(fp);
        if (recovered && recovered !== el) {
          try {
            el.removeAttribute('data-cloudl-ref');
            recovered.setAttribute('data-cloudl-ref', ref);
            identity[ref] = fingerprintOf(recovered);
          } catch (_) {}
          window.__resolved = recovered;
        ...
        return {
          ...
          hint: 'The page has changed since the last snapshot. Re-run \`cloudl browser state\` to refresh.',
        };
      }
      ...
      // ── CSS selector path (any non-numeric input) ──
      {
        ...
        if (matches.length === 0) {
          return {
            ...
            hint: 'The element may not exist or may be hidden. Re-run \`cloudl browser state\` to check, or use \`cloudl browser find --css\` to explore candidates.',
            matches_n: 0,
          };
        }
        ...
        if (matches.length > 1 && !firstOnMulti) {
          ...
          return {
            ...
            hint: 'Pass --nth <n> (0-based) to pick one, or use a more specific selector. Use \`cloudl browser find --css\` to list all candidates.',
