...
function profileConfigPath(): string {
  const baseDir = process.env.CLOUDL_CONFIG_DIR || path.join(os.homedir(), '.cloudl');
...
export type ProfileSelection = {
   ...
   * or CLOUDL_PROFILE env): route strictly and fail loud if it is offline.
...
export function resolveProfileSelection(profile?: string): ProfileSelection | undefined {
  ...
  const explicit = normalizeContextId(profile) ?? normalizeContextId(process.env.CLOUDL_PROFILE);
