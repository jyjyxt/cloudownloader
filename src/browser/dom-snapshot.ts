  ...
  function walkIframe(el, depth) {
    ...
    try {
      ...
      if (!doc || !doc.body) {
        ...
        lines.push(indent + '|iframe|' + frameLabel + '<iframe' + (attrs ? ' ' + attrs : '') + ' /> (cross-origin, use: cloudl browser frames + browser eval --frame <index>)');
        crossOriginIndex++;
        return false;
      }
      iframeCount++;
      ...
      const attrs = serializeAttrs(el);
      const frameLabel = '[F' + crossOriginIndex + ']';
      lines.push(indent + '|iframe|' + frameLabel + '<iframe' + (attrs ? ' ' + attrs : '') + ' /> (blocked, use: cloudl browser frames + browser eval --frame <index>)');
      crossOriginIndex++;
      return false;
    }
  }
