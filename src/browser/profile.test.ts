...
describe('profile selection (requirement vs preference)', () => {
  ...
  beforeEach(() => {
    configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cloudl-profile-test-'));
  ...
  it('tags CLOUDL_PROFILE env as explicit', () => {
    vi.stubEnv('CLOUDL_PROFILE', 'pavmrekj');
  ...
  it('explicit argument beats env beats config default', () => {
    vi.stubEnv('CLOUDL_PROFILE', 'from-env');
