...
async function setFileInputFiles(tabId, files, selector) {
	...
	const rootNodeId = (await sendDebuggerCommand({ tabId }, "DOM.getDocument", {})).root?.nodeId;
	if (typeof rootNodeId !== "number") throw new Error("DOM.getDocument returned no root node");
	const nodeId = (await sendDebuggerCommand({ tabId }, "DOM.querySelector", {
		nodeId: rootNodeId,
		selector: query
	})).nodeId;
	if (typeof nodeId !== "number" || nodeId <= 0) throw new Error(`No element found matching selector: ${query}`);
	try {
		...
		if (!(err instanceof Error ? err.message.toLowerCase() : String(err).toLowerCase()).includes("not allowed")) throw err;
	}
	...
	switch (cmd.op) {
		...
		case "close": {
			...
			if (cmd.page !== void 0 && session?.owned) {
				...
				if (session.preferredTabId === tabId) await releaseLease(leaseKey, "tab close");
				else {
