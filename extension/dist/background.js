...
async function setFileInputFiles(tabId, files, selector) {
  ...
  const document = await sendDebuggerCommand({ tabId }, "DOM.getDocument", {});
...
async function handleCdp(cmd, leaseKey) {
  ...
  try {
    ...
    const data = routeFrameId ? await sendCommandInFrameTarget(tabId, routeFrameId, cmd.cdpMethod, stripCloudlFrameRoutingParams(params, true), aggressive, commandCdpTimeoutMs(cmd) ?? 3e4, routeTargetUrl) : await sendDebuggerCommand(
      { tabId },
      cmd.cdpMethod,
...
function stripCloudlFrameRoutingParams(params, stripFrameId) {
