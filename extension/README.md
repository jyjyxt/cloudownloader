# cloudl Browser Bridge Extension

The extension connects Chrome tabs to the local cloudl daemon. It uses Chrome
extension APIs only as a transport and browser-control layer for explicit CLI
commands.

## Permission Notes

- `debugger`: sends CDP commands to cloudl-controlled or bound tabs.
- `tabs` / `tabGroups`: manages the dedicated cloudl automation container and
  reports selected tab metadata back to the CLI.
- `cookies`: reads cookies for browser-backed adapters that need authenticated
  fetches.
- `downloads`: surfaces download lifecycle to `cloudl browser wait download`.
  The extension observes started / in-progress / completed / failed downloads so
  the CLI can wait for a file triggered by an automation command. cloudl
...
> (started / in-progress / completed / failed) to the cloudl command-line tool,
