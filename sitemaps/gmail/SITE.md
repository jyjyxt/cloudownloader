...
Gmail desktop mail UI. Prefer `cloudl gmail ...`: reads use Gmail's own search/navigation and intercept natural responses. This sitemap does not define write actions because no supported browser-session API contract is currently available.
...
- `/mail/u/<account>/#settings/labels` → settings labels, read by `cloudl gmail labels`
...
- inspect identity → `cloudl gmail whoami`
